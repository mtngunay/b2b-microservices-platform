...
if ($mssqlReady) {
    ...
    $migrationSql = Join-Path $projectRoot "migration.sql"
    
    # Mevcut migration.sql dosyasını kullan
    if (Test-Path $migrationSql) {
        ...
        Write-Host "  migration.sql bulunamadı, atlanıyor..." -ForegroundColor Yellow
    }
    
    # Seed data ekle (admin kullanıcısı)
    Write-Host "  MSSQL seed data ekleniyor..." -ForegroundColor Gray
    $seedSql = @"
-- Admin123! şifresinin SHA256 hash'i
IF NOT EXISTS (SELECT 1 FROM Users WHERE Email = 'admin@demo.com')
BEGIN
    DECLARE @PasswordHash NVARCHAR(512) = 'PrP+ZrMeO00Q+nC1ytSccRIpSvauTkdqHEBRVdRaoSE='
    DECLARE @TenantId NVARCHAR(50) = 'default'
    DECLARE @UserId UNIQUEIDENTIFIER = NEWID()
    DECLARE @RoleId UNIQUEIDENTIFIER = NEWID()
    DECLARE @TenantGuid UNIQUEIDENTIFIER = NEWID()

    -- Tenant oluştur
    IF NOT EXISTS (SELECT 1 FROM Tenants WHERE Subdomain = 'default')
    BEGIN
        INSERT INTO Tenants (Id, Name, Subdomain, IsActive, CreatedAt, ContactEmail, IsDeleted, Settings)
        VALUES (@TenantGuid, 'Default Tenant', 'default', 1, GETUTCDATE(), 'admin@demo.com', 0, '{}')
    END

    -- Admin rolü oluştur
    IF NOT EXISTS (SELECT 1 FROM Roles WHERE Name = 'Admin' AND TenantId = @TenantId)
    BEGIN
        INSERT INTO Roles (Id, Name, Description, IsSystemRole, TenantId, CreatedAt, CreatedBy, IsDeleted)
        VALUES (@RoleId, 'Admin', 'System Administrator', 1, @TenantId, GETUTCDATE(), 'system', 0)
    END
    ELSE
    BEGIN
        SELECT @RoleId = Id FROM Roles WHERE Name = 'Admin' AND TenantId = @TenantId
    END

    -- Admin kullanıcısı oluştur
    INSERT INTO Users (Id, Email, PasswordHash, FirstName, LastName, IsActive, TenantId, CreatedAt, CreatedBy, IsDeleted)
    VALUES (@UserId, 'admin@demo.com', @PasswordHash, 'Admin', 'User', 1, @TenantId, GETUTCDATE(), 'system', 0)

    -- Kullanıcıya rol ata
    INSERT INTO UserRoles (UserId, RoleId, AssignedAt, AssignedBy)
    VALUES (@UserId, @RoleId, GETUTCDATE(), 'system')
...
END
"@
    $seedSql | Out-File -FilePath "$env:TEMP\seed.sql" -Encoding UTF8
    & $kubectl cp "$env:TEMP\seed.sql" b2b-data/mssql-0:/tmp/seed.sql 2>&1 | Out-Null
    & $kubectl exec -it mssql-0 -n b2b-data -- /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P "YourStrong@Passw0rd" -C -d B2BWriteDb -i /tmp/seed.sql 2>&1 | Out-Null
    Remove-Item "$env:TEMP\seed.sql" -Force -ErrorAction SilentlyContinue
    Write-Host "  MSSQL seed tamamlandı (admin@demo.com / Admin123!)" -ForegroundColor Gray
}
...
Write-Host "  API Login:" -ForegroundColor Cyan
